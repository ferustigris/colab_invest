...
class TrailingAnnualDividendYield(FinancialMetric):
    ...
    def get_load_for_ticker(self):
        ...
        if not self.yahoo_data.get('trailingAnnualDividendYield'):
            logger.debug(f"trailingAnnualDividendYield data not available for {self.stock_details.ticker}")
            self.data_quality = 0.0
            self.comment += "\n - trailingAnnualDividendYield data not available"
            return

        now = datetime.now()
        self.value = self.yahoo_data['trailingAnnualDividendYield']
