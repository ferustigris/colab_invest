...
class CurrentPrice(FinancialMetric):
    ...
    def get_load_for_ticker(self):
        ...
        if not 'currentPrice' in self.yahoo_data:
        ...
        now = datetime.now()
        self.value = self.yahoo_data['currentPrice']
        ...
        try:
            ...
            now = datetime.now()
            self.data_quality = 1.0/((now - self.yahoo_data_last_update_dt).days // 1 + 1) if self.value else 0.0
