...
class ForwardAnnualDividendYield(FinancialMetric):
    ...
    def get_load_for_ticker(self):
        ...
        if not self.yahoo_data.get('forwardAnnualDividendYield'):
            logger.debug(f"forwardAnnualDividendYield data not available for {self.stock_details.ticker}")
            self.data_quality = 0.0
            self.comment += "\n - forwardAnnualDividendYield data not available"
            return
        ...
        self.value = self.yahoo_data['forwardAnnualDividendYield'] / 100  # Already percentage
