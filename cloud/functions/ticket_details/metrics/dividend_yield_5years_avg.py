...
class FiveYearAvgDividendYield(FinancialMetric):
    ...
    def get_load_for_ticker(self):
        ...
        if not self.yahoo_data.get('fiveYearAvgDividendYield'):
            logger.debug(f"fiveYearAvgDividendYield data not available for {self.stock_details.ticker}")
        ...
        self.value = self.yahoo_data['fiveYearAvgDividendYield'] / 100 # Already percentage
