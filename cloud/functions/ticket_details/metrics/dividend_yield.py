...
class DividendYield(FinancialMetric):
    ...
    def get_load_for_ticker(self):
        ...
        if not self.yahoo_data.get('dividendYield'):
            logger.debug(f"dividendYield data not available for {self.stock_details.ticker}")
        ...
        self.value = self.yahoo_data['dividendYield'] / 100.0  # Convert percentage to decimal
