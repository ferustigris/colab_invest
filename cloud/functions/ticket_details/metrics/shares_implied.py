...
class SharesImplied(FinancialMetric):
    ...
    def get_load_for_ticker(self):
        ...
        if not 'impliedSharesOutstanding' in self.yahoo_data or self.yahoo_data['impliedSharesOutstanding'] is None:
            logger.debug(f"impliedSharesOutstanding data not available for {self.stock_details.ticker}")
        ...
        now = datetime.now()
        self.value = self.yahoo_data['impliedSharesOutstanding']
        ...
        logger.info(f"ImpliedSharesOutstanding metric loaded successfully: value={self.value:.2f}, quality={self.data_quality:.2f}")
